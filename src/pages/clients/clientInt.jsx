...
function AddPaymentDialog({ open, onClose, onSave, billingId, netTotal, alreadyPaid, clientId, clientName, accessToken }) {
  ...
  const handleSave = async () => {
    const base = {
      ...
      preparedBy:    accessToken?.username || accessToken?.name || accessToken?.EmployeeName || "system",
    };
          ...
          <Table stickyHeader size="small">
            ...
            <TableBody>
              {payments.length === 0 ? (
                ...
                <TableRow key={i} hover sx={{ backgroundColor: i % 2 === 0 ? "transparent" : "action.hover" }}>
                  <TableCell sx={{ ...cellSx, color: "text.disabled", fontSize: "0.72rem", width: 40 }}>{i + 1}</TableCell>
                  <TableCell sx={cellSx}>{p.ClientID || "—"}</TableCell>
                  <TableCell sx={cellSx}>
                    <Chip label={p.PaymentMethod || "—"} size="small" color={methodColor(p.PaymentMethod)} sx={{ fontSize: "0.7rem" }} />
                  </TableCell>
                  ...
                  <TableCell sx={cellSx}>
                    <Chip label={p.Status || p.PaymentStatus || "—"} size="small"
                      color={statusColor(p.Status || p.PaymentStatus)}
                      sx={{ fontSize: "0.7rem" }} />
                  </TableCell>
                </TableRow>
              ))}
